...
static VALUE ExtLib_Load(VALUE self, VALUE path)
{
	...
	newLib = dlLoadLibrary(RSTRING(path)->ptr);
	if(newLib != NULL) {
		dlFreeLibrary(extLib->lib);
		extLib->lib = newLib;
...
static VALUE ExtLib_Each(int argc, VALUE* argv, VALUE self)
{
	rb_dcLibHandle* extLib;
	size_t i, c;
...
static VALUE ExtLib_Call(int argc, VALUE* argv, VALUE self)
{
	...
	if(argc != RSTRING(argv[1])->len)	/* Don't count the return value in the signature @@@ write something more secure */
	...
	sig = RSTRING(argv[1])->ptr;/*@@@ should use StringValuePtr for real strings... 'S' in signature*/
...
/* @@@ Allow conversion of Bignum, too. */
			case DC_SIGCHAR_CHAR:     if(b = (t == T_FIXNUM)) dcArgChar    (cvm, (DCchar)    FIX2LONG(argv[i]));      break;
			case DC_SIGCHAR_SHORT:    if(b = (t == T_FIXNUM)) dcArgShort   (cvm, (DCshort)   FIX2LONG(argv[i]));      break;
			case DC_SIGCHAR_INT:      if(b = (t == T_FIXNUM)) dcArgInt     (cvm, (DCint)     FIX2LONG(argv[i]));      break;
			case DC_SIGCHAR_LONG:     if(b = (t == T_FIXNUM)) dcArgLong    (cvm, (DClong)    FIX2LONG(argv[i]));      break;
			case DC_SIGCHAR_LONGLONG: if(b = (t == T_FIXNUM)) dcArgLongLong(cvm, (DClonglong)FIX2LONG(argv[i]));      break;
			case DC_SIGCHAR_FLOAT:    if(b = (t == T_FLOAT))  dcArgFloat   (cvm, (DCfloat)   RFLOAT(argv[i])->value); break;
			case DC_SIGCHAR_DOUBLE:   if(b = (t == T_FLOAT))  dcArgDouble  (cvm, (DCdouble)  RFLOAT(argv[i])->value); break;

			case DC_SIGCHAR_POINTER:
				b = 1;	
				switch(t) {
					case T_STRING: dcArgPointer(cvm, RSTRING(argv[i])->ptr); break;  /* String. */
					default:       b = 0;                                    break;
				}
				break;
	...
	switch(sig[i-1]) {
		...
		case DC_SIGCHAR_CHAR:     r = CHR2FIX(     dcCallChar    (cvm, fptr));                 break;
		case DC_SIGCHAR_SHORT:    r = INT2FIX(     dcCallShort   (cvm, fptr));                 break;
		case DC_SIGCHAR_INT:      r = INT2FIX(     dcCallInt     (cvm, fptr));                 break;
		case DC_SIGCHAR_LONG:     r = INT2FIX(     dcCallLong    (cvm, fptr));                 break;
		case DC_SIGCHAR_LONGLONG: r = INT2FIX(     dcCallLongLong(cvm, fptr));                 break;
