...
and wrap the C interface as a newly attached R name space that uses the same symbolic names of the C API.
...
locate the C library.\footnote{Possible C math library names are \file{libm.so} and \file{MSVCRT.DLL} 
in locations such as \file{/lib}, \file{/usr/lib}, \file{/lib64}, \file{/usr/lib64}, \file{C:\textbackslash WINDOWS\textbackslash SYSTEM32} etc..}
...
or the C Math library labeled \file{m} otherwise.

\begin{example}
> mLib <- dynfind(c("msvcrt","m"))
\end{example}
...
a new R name space \citep{RNameSpace} will be populated with thin R wrapper
...
This paper introduces the \pkg{rdyncall} package (Version 0.7.2 on CRAN as of this writing) that contributes an improved Foreign Function Interface for R.
